...
func (s *Store) Create(input Input) (Tag, error) {
...
`, t.ID, t.Name, t.Color, t.CreatedAt, t.UpdatedAt)
	if err != nil {
		if isUniqueViolation(err) {
			return Tag{}, fmt.Errorf("%w: tag name already exists", ErrConflict)
		}
		return Tag{}, err
	}
	return t, nil
...
func (s *Store) Update(id string, input Input) (Tag, error) {
	name, color, err := validate(input)
	if err != nil {
		return Tag{}, err
	}
...
func validate(input Input) (string, string, error) {
	...
	if !isHexColor(color) {
		return "", "", fmt.Errorf("%w: color must be a hex value like #0ea5e9", ErrValidation)
	}
